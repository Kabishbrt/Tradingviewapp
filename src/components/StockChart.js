...
const StockChart = ({}) => {
    ...
    // const sortedChartData = chartData.chart.sort((a, b) => {
    ...
    //     } else {
    //         // If timestamps are equal, use the index as a secondary sorting criterion
    //         return a.index - b.index;
    //     }
    // });
    
    
    // let updatedCandledata = sortedChartData.map((data, index) => {
    //     const { date, ...rest } = data;
    //     const formattedDate = new Date(date).toISOString().split('T')[0]; // Extract YYYY-MM-DD
    
    //     if (index === 1258) {
    //         // Skip processing for index 1258
    //         console.log("Data at Index 1258:", date, formattedDate);
    //         return null;
    //     }
    
    //     return { time: formattedDate, ...rest };
    // }).filter(data => data !== null); // Filter out the null values (skipped indices)
    
    // console.log(updatedCandledata);
    
    // Sort the data by the time property in ascending order
    ...
    const [chartColors, setChartColors] = useState({
        background: darkMode ? '#152133' : '#ffffff',
        textColor: darkMode ? '#ffffff' : '#000000',
        vertLinesColor: darkMode ? '#24344d' : 'rgba(197, 203, 206, 0.5)',
        horzLinesColor: darkMode ? '#24344d' : 'rgba(197, 203, 206, 0.5)',
        borderColor: darkMode ? 'rgba(255, 255, 255, 0.8)' : 'rgba(197, 203, 206, 0.8)',
        upColor: darkMode ? 'rgba(0, 128, 0, 1)' : 'rgba(0, 128, 0, 1)',
        downColor: darkMode ? 'rgba(255, 0, 0, 1)' : 'rgba(255, 0, 0, 1)',
        borderDownColor: darkMode ? 'rgba(255, 0, 0, 1)' : 'rgba(255, 0, 0, 1)',
        borderUpColor: darkMode ? 'rgba(0, 128, 0, 1)' : 'rgba(0, 128, 0, 1)',
        wickDownColor: darkMode ? 'rgba(255, 0, 0, 1)' : 'rgba(255, 0, 0, 1)',
        wickUpColor: darkMode ? 'rgba(0, 128, 0, 1)' : 'rgba(0, 128, 0, 1)',
        priceScaleBorderColor: darkMode ? '#666666' : '#ccc'
    });
    ...
    useEffect(() => {
        ...
        const chart = createChart(chartContainer, {
            ...
            grid: {
                vertLines: {
                    color: chartColors.vertLinesColor,
                },
                horzLines: {
                    color: chartColors.horzLinesColor,
                },
            },
            crosshair: {
                mode: 0,
            ...
            priceScale: {
                ...
                alignLabels: true,
                borderVisible: true,
                borderColor: chartColors.priceScaleBorderColor,
                scaleMargins: {
                    top: 0.1,
                    bottom: 0.1,
                },
  ...
  return (
    <div id="chart-container" className="flex flex-row mt-2 justify-start">
    </div>
  );
};
